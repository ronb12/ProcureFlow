...
export function PolicyCheckItem({ check, className }: PolicyCheckItemProps) {
  ...
  const bgColor = check.severity === 'error' ? 'bg-destructive/10' : 'bg-yellow-50';
  const borderColor = check.severity === 'error' ? 'border-destructive/20' : 'border-yellow-200';
  const textColor = check.severity === 'error' ? 'text-destructive' : 'text-yellow-800';
  const iconColor = check.severity === 'error' ? 'text-destructive' : 'text-yellow-600';
...
export function PolicyCheckSummary({ checks, className }: PolicyCheckSummaryProps) {
